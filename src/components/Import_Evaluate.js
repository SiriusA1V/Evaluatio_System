...
const LIST_COL = constants.LIST_COL2;

class Import_Evaluate extends Component{
    state = {
        insert_info : [],
        is_button : true
    }
        ...
        if(input.files[0] !== undefined){
          ...
          reader.onload = () => {
            ...
            for(var i = 0; i < save_arr.length; i++){
              ...
              if(i !== save_arr.length-1 && save_arr[i].length !== 55){
                alert_num = 1;
              }
              if(save_arr[i].length === 55 && save_arr[i][0].length === 0){
                alert_num = 1;
              }
            }
  
            if(save_arr[0][0] !== "期" || save_arr[0][1] !== "半期" || save_arr[0][2] !== "ID" || save_arr[0][3] !== "氏名"){
              alert_num += 2;
            }
            if(save_arr[save_arr.length-1].length !== 55 && save_arr[save_arr.length-1].length !== 1){
              alert_num += 4;
            }
            ...
            save_arr.shift();
            if(save_arr[save_arr.length-1].length !== 55){
                save_arr.pop();
            }
    ...
    onClick_submit=()=>{
        ...
        axios({
            method: 'post',
            url: SERVER_URL+"/insert_evaluate",
...
export default Import_Evaluate;